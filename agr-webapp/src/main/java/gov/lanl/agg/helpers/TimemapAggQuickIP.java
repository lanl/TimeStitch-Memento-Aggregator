...
*/
public class TimemapAggQuickIP {
	static List tglist;
	static Map hmap;
	static Map plist;
	...
	public void getTimeMapInfo (List <ArchiveDescription> timemaplist,String url) {
		  ...
		  CompletionService<Map> ecs = new ExecutorCompletionService<Map> (exec);
		   client = (HttpClient) MyInitServlet.getInstance().getAttribute("httpcli_ip");
		 ...
		 // System.out.println("endSeconds"+ endSeconds);
		  List <TimeMapTaskIP> tasks = new ArrayList<TimeMapTaskIP> (); 
	    	   ...
	    	   Iterator<ArchiveDescription> di =  timemaplist.iterator();
			    while (di.hasNext()) {
			    	...
			    	//tasks.add(new TimeGateTask(turl,date));
			    	  if (openwayback!=null) {
			    		  tasks.add(new TimeMapTaskIP(od,tmap+enurl, client,mprefix,tmap));  
			    	  }
			    	  else {
			    	  tasks.add(new TimeMapTaskIP(od,turl,client));
			    	  }
			    ...
			    //check if native resource exists
	              ...
	              // int nn = timemaplist.size();
	              // int nat= -1;
	               TimeGateClientIP tgclient = new TimeGateClientIP();
	               RunMeBatchTask rmtask = (RunMeBatchTask) MyInitServlet.getInstance().getAttribute("task");
		              
	            //   HttpClient    httpclient =(HttpClient) MyInitServlet.getInstance().getAttribute("httpcli_ip");
	               tgclient.checkFeedUrl(  client, eurl,MementoUtils.httpformatter.format(new Date()));
	               OriginalResource ores = tgclient.getOriginalResource();
	               if (ores.getTimeMapIndexURI()!=null) {
	            	   String tmstr = ores.getTimeMapIndexURI();
                 	  tmstr =  tmstr.replace(eurl,"");
                 	 ...
                 	 if (!tmlist.contains(tmstr)) {
	            	   ...
	            	   tasks.add(new TimeMapTaskIP(nat, ores.getTimeMapIndexURI(),client));
	            	   n=n+1;
                 	 }
	                      ...
	                      if (ores.getTimeMapURI()!=null) {
	            	          ...
	            	          rmtask.addArchive(ihost, ores.getTimeGateURI(),tmstr,name,null);
	            	          tasks.add(new TimeMapTaskIP(nat, ores.getTimeMapURI(),client));
	            	          n=n+1;
	                    	  }
	    	    ...
	    	    List<Future<Map>> futures  = new ArrayList<Future<Map>>();
	    	 
	    	 try {
	                        for (TimeMapTaskIP s : tasks) {
	                       ...
	                       for (int i = 0; i < n; ++i) {
	                	        ...
	                	        long timeLeft = endSeconds-TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis());
	                	         ...
	                	         if (result == null) {
	                              ...
	                              rr = ( (Map) ecs.take().get());
	                              Integer hostid = (Integer) rr.keySet().iterator().next();
        	                     
       	                           r= (LinkHeader) rr.get(hostid.intValue());
	                    	                      ...
	                    	                      String pstatus="0";
	        	                                   ...
	        	                                   if (pstatus.equals("3")) {
	                 	                    		 ...
	                 	                    		 if (ntimemap!=null) {
	                 	                    		  ...
	                 	                    		  TimeMapTaskIP task = new TimeMapTaskIP(hostid,ntimemap,client);
	                 	                    	  ...
	                 	                    	  if (pstatus.equals("666")) {
	                 	                    		 ...
	                 	                    		 String ntimemap = null;
	                 	                    		  ...
	                 	                    		  TimeMapTaskIP task= new TimeMapTaskIP(hostid,ntimemap,client,mprefix, tmap);
	                 	                    		  futures.add(ecs.submit(task));
	                 	                    		  n=n+1;
	                 	                    		  }
	                 	                    	  }
	 		...
	 		LinkHeader linkheader = null;
	 		 try {
                       ...
                       method.getParams().setCookiePolicy(CookiePolicy.IGNORE_COOKIES);
	 		           ...
	 		           System.out.println("url in call():"+url);
	 		           int statusCode=0;
