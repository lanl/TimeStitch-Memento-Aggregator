...
public class PredictResourceML {
	static URI baseUri;
	...
	public PredictResourceML(@Context UriInfo uriInfo) {
	 ...
	 * //@Produces({"application/link-format",MediaType.TEXT_PLAIN}) public
	 * Response getMyLinks(@PathParam("id") String idp, @PathParam("page")
	 * String page, @Context UriInfo ui, @Context HttpHeaders hh) throws
	 * ParseException { URI ur = ui.getRequestUri(); System.out.println(
	 * "request timemap url:"+ur.toString());
	 * 
	 * List <String> hscheme = hh.getRequestHeader("X-Forwarded-Proto");
	 * 
	 * if (hscheme==null) { protocol ="http://"; } else { protocol ="https://";
	 * }
	 * 
	 * 
	 * 
	 ...
	 * 
	 * URI baseurl = ui.getBaseUri(); //if (baseurl.getScheme().equals("https"))
	 * { // protocol = "https://"; //}
	 * System.out.println("baseurl"+baseurl.toString()); String id; boolean
	 * paging =false; String noindex = page; if (page.equals("a")) { id =
	 * ur.toString().replaceFirst(baseurl.toString()+"timemapml/link/", "");
	 * page = "1"; } else { id =
	 * ur.toString().replaceFirst(baseurl.toString()+"timemapml/link/"+page+"/",
	 * ""); } System.out.println("page"+page); //
	 * System.out.println("livestatus:"+livestatus);
	 * 
	 * if (id.startsWith("https:")) { id = id.replaceFirst("https:", "http:"); }
	 * 
	 * check_cache_header(hh);
	 * 
	 * // String sthost=""; boolean skip = false;
	 * 
	 * boolean redirect = false; boolean refuse = false; CacheStats stats =
	 * (CacheStats) MyInitServlet.getInstance().getAttribute("timemapstats");
	 * CommonRuleMatcher urlmatcher = new CommonRuleMatcher(); List archivelist
	 * = urlmatcher.getArchives(id,tgrulesdynamic);
	 * System.out.println("tm:"+archivelist.toString());
	 * 
	 * String redlocation =""; if (archivelist.size()==1) { Iterator it =
	 * adesc.iterator(); while (it.hasNext()) { ArchiveDescription ad =
	 * (ArchiveDescription) it.next(); if (archivelist.contains(ad.getName())){
	 * String mredirect = ad.getTimemapRedirect(); if (mredirect.equals("yes"))
	 * redirect = true; if (mredirect.equals("refuse")) refuse = true;
	 * 
	 * redlocation = ad.getTimemap(); break; } } }
	 * 
	 * String sthost=null;
	 * 
	 * 
	 * label: try {
	 * 
	 * id = mc.validateAndEncodeUrl(id); if (id==null) { skip = true; break
	 * label; }
	 * 
	 * URL turl = new URL(id); sthost = turl.getHost(); if
	 * (sthost.indexOf("web.archive.org")>0) { skip = true; break label; } if
	 * (sthost.indexOf("http://")>0) { skip = true; break label; } if
	 * (id.length()>1024) skip = true; break label; } catch ( Exception ignore)
	 * { ignore.printStackTrace(); System.out.println("url bad tg:"+id);
	 * skip=true; }
	 * 
	 * 
	 * 
	 * if (MementoUtils.isDomainBlacklisted(sthost, (HashMap<String, String>)
	 * MyInitServlet.getInstance().getAttribute("blacklistDomains"))) { //
	 * Blacklisted Domain -- 403 System.out.println("Blacklisted Domain: " +
	 * id);
	 * 
	 * Response.ResponseBuilder responseBuilder = Response.status(403);
	 * responseBuilder.entity(MementoUtils.composeErrorPage403(id)); return
	 * responseBuilder.build(); }
	 * 
	 * 
	 * if (refuse) { //bad url ResponseBuilder r = Response.status(404).entity(
	 * "service not provided for url: " + id); r.header("Content-Type",
	 * "text/plain"); return r.build(); }
	 * 
	 * if (skip) { //bad url ResponseBuilder r = Response.status(400).entity(
	 * "bad url: " + id); r.header("Content-Type", "text/plain"); return
	 * r.build(); } ////notify batch about request System.out.println(
	 * "no notify"); //notify_batch(id); if (redirect) {
	 * do_redirect(redlocation,id); }
	 * 
	 * String bu=baseUri.toString(); if (proxybaseuri!=null){ bu =
	 * protocol+proxybaseuri; }
	 * 
	 * int istart=0; if (isNumeric(page)) { istart =Integer.parseInt(page)-1; }
	 * 
	 * List f_archives = urlmatcher.getArchives(id,tgrulescache); if
	 * (sthost.startsWith("www.")) { sthost=sthost.replaceFirst("www.", ""); }
	 * List filtered_archives = new ArrayList();
	 * filtered_archives.addAll(f_archives); if
	 * (!filtered_archives.contains(sthost)){ filtered_archives.add(sthost); }
	 * System.out.println("tm cache list:"+filtered_archives.toString());
	 * 
	 * stats.incrementHit(); System.out.println("timemap live:"+id); return
	 * compose_static_page( bu, id, archivelist,sthost);
	 * 
	 * }
	 * 
	 * public Response make_503(String id) { ResponseBuilder r =
	 * Response.status(503).entity("Retry-After: 30 sec for url: " + id);
	 * r.header("Link","<"+id+">;rel=\"original\"\n"); r.header("Content-Type",
	 * "text/plain"); r.header("Retry-After", 30); return r.build(); } public
	 * Response make_504(String id) { ResponseBuilder r =
	 * Response.status(504).entity("Not cached: " + id);
	 * r.header("Link","<"+id+">;rel=\"original\"\n"); r.header("Content-Type",
	 * "text/plain"); return r.build(); }
	 * 
	 * //notify batch about request public void notify_batch(String id){ try {
	 * IBigQueue ressyncque = (IBigQueue)
	 * MyInitServlet.getInstance().getAttribute("ResSyncQue"); Date now = new
	 * Date(); if (ressyncque!=null) { String pload = now.getTime()+"|"+ id;
	...
	public Response getMyJsonLinks(@PathParam("id") String idp, @PathParam("page") String page, @Context UriInfo ui,
			@Context HttpHeaders hh) throws ParseException {
		...
		if (page.equals("a")) {
			id = ur.toString().replaceFirst(baseurl.toString() + "prediction/json/", "");
			page = "1";
		} else {
			id = ur.toString().replaceFirst(baseurl.toString() + "prediction/json/" + page + "/", "");
		}
		System.out.println("page" + page);
		...
		if (archivelist.size() == 1) {
			...
			while (it.hasNext()) {
				...
				if (archivelist.contains(ad.getName())) {
					...
					if (mredirect.equals("yes"))
						redirect = true;
					if (mredirect.equals("refuse"))
						refuse = true;
		...
		if (MementoUtils.isDomainBlacklisted(sthost,
				(HashMap<String, String>) MyInitServlet.getInstance().getAttribute("blacklistDomains"))) {
			System.out.println("Blacklisted Domain: " + id);
			return Response.status(403).entity(MementoUtils.composeErrorPage403(id)).build();
		}

		if (refuse) {
			// bad url
			ResponseBuilder r = Response.status(404).entity("service not provided for url: " + id);
			return r.header("Content-Type", "text/plain").build();
		}

		if (skip) {
			return Response.status(400).entity("bad url: " + id).header("Content-Type", "text/plain").build();
	...
	public ResponseBuilder do_dynamic_lookup(String id, List archivelist, int istart, String bu, String page,
			List filtered_archives) throws ParseException {
		...
		if (links.size() < pagesize + 1) {
			...
			sb.append(",\n <" + bu + "timemap/link/" + id + ">;rel=\"self\"; type=\"application/link-format\"; from =\""
	...
	public Response json_dynamic_lookup(String id, List archivelist, int istart, String bu, String page,
			List filtered_archives) throws ParseException {
		TimemapAggQuick tgq = new TimemapAggQuick();
		Iterator<ArchiveDescription> ait = adesc.iterator();
		List<ArchiveDescription> tglistp = new ArrayList<ArchiveDescription>();
		...
		if (links.size() < pagesize + 1) {
			...
			for (int i = 0; i < links.size(); i++) {
				...
				// sb.append(", <"+ l.getHref()+">;rel=\"memento"+ type+"\";
				// datetime=\"" +
				// MementoUtils.httpformatter.format(fd)+ "\"\n");
			...
			if (tmmap.size() == 0) {
				...
				// sbn.append("\"timegate_uri\":\""+bu +"timegate/"+
				// id+"\",\n");
				// sb.append(" , <"+bu +"timegate/"+ id+">;rel=\"timegate\"");
				Date fromself = null;
	 ...
	 * @GET
	 * 
	 * @Path("link/{id:.*}") //@Produces("application/link-format" ) public
	 * Response getDefaultLinks(@PathParam("id") String idp, @Context UriInfo
	 * ui, @Context HttpHeaders hh) throws ParseException { String page = "a";
	 * List <String> hscheme = hh.getRequestHeader("X-Forwarded-Proto");
	 * 
	 * if (hscheme==null) { protocol ="http://"; } else { protocol ="https://";
	 * } //return getMyLinks(idp,page, ui) ; return getIndexLinks(idp,page,
	 * ui,hh) ;
	 * 
	 * }
	 */
	...
	@Path("json/{id:.*}")
	// @Produces("application/json" )
	public Response getJLinks(@PathParam("id") String idp, @Context UriInfo ui, @Context HttpHeaders hh)
	...
	public Response getIndexLinksJson(@PathParam("id") String idp, @PathParam("page") String page, @Context UriInfo ui,
			@Context HttpHeaders hh) throws ParseException {
		URI ur = ui.getRequestUri();
		System.out.println("request timemap url:" + ur.toString());
		URI baseurl = ui.getBaseUri();
		...
		String rdate = MementoUtils.httpformatter.format(new Date());
		Date d = MementoUtils.httpformatter.parse(rdate);
		String tgdate = MementoUtils.timeTravelMachineFormatter.format(d);
		check_cache_header(hh);
		String sthost = "";
		boolean skip = false;
		boolean redirect = false;
		...
		stats.incrementHit();
		// notify batch about request
		((CacheStorage) MyInitServlet.getInstance().getAttribute("storage")).checkCache(id, d, false, service_date);
		return compose_json_page(bu, id, archivelist, sthost);
	...
	public Response getIndexLinks(@PathParam("id") String idp, @PathParam("page") String page, @Context UriInfo ui,
			@Context HttpHeaders hh) throws ParseException {
		URI ur = ui.getRequestUri();
		System.out.println("request timemap url:" + ur.toString());
		URI baseurl = ui.getBaseUri();
	...
	public Response do_redirect(String redlocation, String id) {
		...
		// String location = "http://mementoweb.org/proxy/wiki/timemap/link/"
		// +id;
	...
	public String getNativeTimemap(String id) {
		...
		if (ores.getTimeMapIndexURI() != null) {
			nativetimegate = ores.getTimeGateURI();
	...
	public Response compose_internal_page(String bu, String id, List archivelist, String ihost) {
		...
		while (it.hasNext()) {
			...
			if (mstatus.contains("yes")) {
				sb.append(",\n <" + ard.getTimemapGlobal() + id + ">;rel=\"timemap\"; "
						+ "type=\"application/link-format\"; " + "title=\"memento_compliant:yes|archive_id:" + name
						+ "\"");
			} else {
				sb.append(",\n <" + ard.getTimemapGlobal() + id + ">;rel=\"timemap\"; "
						+ "type=\"application/link-format\"; " + "title=\"memento_compliant:no|archive_id:" + name
						+ "\"");
			}
			// }
		}
		...
		if (nattm != null) {
			...
			sb.append(",\n <" + nattm
					+ ">;rel=\"timemap\"; type=\"application/link-format\"; title=\"memento_compliant:yes|archive_id:"
					+ ihost + "\"");
		}
		sb.append(",\n <" + bu + "timemap/link/" + id + ">;rel=\"self\"; type=\"application/link-format\"\n");
		ResponseBuilder r = Response.ok(sb.toString());
		r.header("Content-Type", "application/link-format");
		return r.build();
	}
	...
	public Response compose_static_page(String bu, String id, List archivelist, String ihost) {
		...
		List excludeArchives = mlclient.checkUrl(httpclient, mlbaseurl + id, new ArrayList());
			...
			if (archivelist.contains(name)) {
				if (!excludeArchives.contains(name)) {
					if (mstatus.equals("yes")) {
						// arcvl.add(ard);
						sb.append(",\n <" + ard.getTimemapGlobal() + id + ">;rel=\"timemap\"; "
								+ "type=\"application/link-format\"; " + "title=\"memento_compliant:yes|archive_id:"
								+ name + "\"");

					} else {
						sb.append(",\n <" + ard.getTimemapGlobal() + id + ">;rel=\"timemap\"; "
								+ "type=\"application/link-format\"; " + "title=\"memento_compliant:no|archive_id:"
								+ name + "\"");
					}
				}
			}
	...
	public Response compose_json_page(String bu, String id, List archivelist, String ihost) {
			...
			if (archivelist.contains(name)) {
				if (!excludeArchives.contains(name)) {
					if (mstatus.equals("yes")) {
						// arcvl.add(ard);
						sb.append("{");
						sb.append("\"timegate_uri\":\"" + ard.getTimegateGlobal() + id + "\",\n");
						sb.append("\"timemap_uri\":\"" + ard.getTimemapGlobal() + id + "\",\n");
						sb.append("\"memento_compliant\":\"yes\",\n");
						sb.append("\"archive_id\":\"" + name + "\"\n");
		...
		if (nattm != null) {
			...
			sb.append("{");
			if (nativetimegate != null) {
				sb.append("\"timegate_uri\":\"" + nativetimegate + "\",\n");
			}
			sb.append("\"timemap_uri\":\"" + nattm + "\",\n");
			sb.append("\"memento_compliant\":\"yes\",\n");
			sb.append("\"archive_id\":\"" + ihost + "\"\n");
