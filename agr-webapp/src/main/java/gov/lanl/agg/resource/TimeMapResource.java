...
public class TimeMapResource {
	  ...
	  String srvdate;
	  Date service_date;
	  boolean stats_incremented = false;
	...
	public void check_prefer_header(HttpHeaders hh){
		 ...
		 String parts [] =preferstr.split(";");
		 if (parts[0].contains("datetimefirst")){
			 int i = parts[0].indexOf("datetimefirst=");
			 
			 startdate = parts[0].substring(i+14);
			 System.out.println("startdate"+startdate);
		 }
		 if (parts[1].contains("datetimelast")){
			 int i = parts[1].indexOf("datetimelast=");
			 
			 enddate = parts[1].substring(i+13);
	...
	public  Response getMyLinks(@PathParam("id") String idp, @PathParam("page") String page, @Context UriInfo ui, @Context HttpHeaders hh) throws ParseException {
		...
		// String protocol = "http://";
		 ...
		 String noindex = page;
		 String logservice = "tmch";
		 if (page.equals("a")) {
			  logservice="tm";
			  id = ur.toString().replaceFirst(baseurl.toString()+"timemap/link/", "");	 
			  page = "1";
		 }
		 else {
		  id = ur.toString().replaceFirst(baseurl.toString()+"timemap/link/"+page+"/", "");
		 }
		...
		boolean refuse = false;
		   ...
		   //check cache first
		     ...
		     if (startdate!=null){
		              ...
		              //reformat it 
		              try {
		              startd = MementoUtils.timeTravelMachineFormatter.parse(startdate);        
		              }
		              catch (Exception e) {
		              	//skip it;
	                    ...
	                    //reformat it 
	                     try {
	                     endd = MementoUtils.timeTravelMachineFormatter.parse(enddate);        
	                     }
	                    catch (Exception e) {
	              	      //skip it;
	              	    System.out.println("bad date");
		    	 ...
		    	 System.out.println("doing partial timemap");
		    	 hlinks = storage.getTimeMapInfo(id, startd, endd, istart, pagesize +1, filtered_archives);
	     ...
	     if (hlinks==null) {	
	  	    	  stats.incrementMiss(); 
	  	    	 if (!stats_incremented){
	  	    	  statsloger.info(id+" "+srvdate+" "+logservice+" 0 1 0 0");
	  	    	  stats_incremented = true;
	  	    	 }
			...
			//hit
			 if (!stats_incremented){
			 statsloger.info(id+" "+srvdate+" "+logservice+ " 1 0 0 0");
			 stats_incremented = true;
			 }
	...
	public  Response getMyJsonLinks(@PathParam("id") String idp, @PathParam("page") String page, @Context UriInfo ui, @Context HttpHeaders hh) throws ParseException {
		...
		// String protocol = "http://";
		 ...
		 if (page.equals("a")) {
			  logservice = "tm";
			  id = ur.toString().replaceFirst(baseurl.toString()+"timemap/json/", "");	 
			  page = "1";
		 }
		 else {
		  id = ur.toString().replaceFirst(baseurl.toString()+"timemap/json/"+page+"/", "");
		 }
		 System.out.println("page"+page);
		 //id = id.indexOf("/")
	...
	public  ResponseBuilder do_dynamic_lookup(String id,List archivelist,int istart,String bu,String page,List filtered_archives) throws ParseException{
     ...
     StringBuffer sb = new StringBuffer("<"+id+">;rel=\"original\"");
           ...
           if (links.size() < pagesize+1 && startdate.equals(null)){
       ...
       return  r;
        	   ...
        	   if (startdate!=null){
		    	 ...
		    	 if (hlinks == null) {
		              ResponseBuilder r = Response.status(404).entity(" not found for url: " + id);
		              r.header("Last-Modified", MementoUtils.httpformatter.format(update));
		              System.out.println("no mementoes in this interval "); 
		              return  r;  
	            }
		    	 
		    	  String type="";
				   //need to remove last entry
		    	  List  <Link> mlinks = hlinks.getLinks();
				  for ( Link l : mlinks) {
				 ...
				 sb.append (",\n <"+bu +"timemap/link/" +selfp + id+
						">;rel=\"self\"; type=\"application/link-format\"; from =\""+
						MementoUtils.httpformatter.format(startd)+"\";until=\""+
						MementoUtils.httpformatter.format(endd)+"\"");
				  //if (!noindex.equals("a")){
				    //        sb.append (",\n <"+bu +"timemap/link/"+ id+
						//	">;rel=\"index\"; type=\"application/link-format\"");
				  //}
		...
		boolean refuse = false;
		   ...
		   System.out.println("tm: dynamic"+archivelist.toString());
	         ...
	         TimeZone tz = TimeZone.getTimeZone("GMT");
			  ...
			  if (lastupdate!=null){
	                if (lastupdate.before(cutoff)){            
	        	         stats.increamentStale(); 
	        	         if (!stats_incremented){
	        				 statsloger.info(id+" "+srvdate+" tm 0 0 1 0");
	        				 stats_incremented = true;
	        				 }
	        	        	    if (onlycached){
		         	        		return make_504(id);
			  ...
			  Date gracedate = getDayOffSet(Calendar.MINUTE,10);
			   if (nocache){
				   ...
				   stats.incrementHit(); //?
				   if (!stats_incremented){
						 statsloger.info(id+" "+srvdate+" tm 0 0 0 1");
						 stats_incremented = true;
						 }
				   //ResponseBuilder rb = json_dynamic_lookup(id,archivelist,istart, bu,page,filtered_archives);
				   return json_dynamic_lookup(id,archivelist,0, bu,page,filtered_archives);
		    ...
		    if( tmmap.size()==0) {
		    	  stats.incrementMiss(); 
		    	  if (!stats_incremented){
		 			 statsloger.info(id+" "+srvdate+" tm 0 1 0 0");
		 			 stats_incremented = true;
		 			 }
     	        	 if (onlycached){
       	        		return make_504(id);
		    ...
		    stats.incrementHit(); 
		    if (!stats_incremented){
				 statsloger.info(id+" "+srvdate+" tm 1 0 0 0");
				 stats_incremented = true;
				 }
			Iterator it = tmmap.keySet().iterator();
			  // StringBuffer sb = new StringBuffer("<"+id+">;rel=\"original\"\n");
		 ...
		 System.out.println("page"+page);
		  //removed  Sept 16, 2016
		   ...
		   System.out.println("tm:"+archivelist.toString());
		     ...
		     HttpClient client = (HttpClient) MyInitServlet.getInstance().getAttribute("httpcli");
	         Date lastupdate = rmtask.checkLastUpdate(id);
	         Date cutoff = getDayOffSet(Calendar.DAY_OF_MONTH,30);
	          if (!nocache) {
			  if (lastupdate==null) {
			      Map m =(Map) rcache.CheckCaches(client, id, sthost, rmtask, storage);
			      if (m!=null){
		   ...
		   // if (maxres>0) {  //need more to test remove for now
		    if ( maxres<pagesize || startdate!=null ) {
		    	System.out.println( "pagesize"+pagesize);
			    page = "a";
	   ...
	   /*  Iterator<ArchiveDescription> ait = adesc.iterator();
	     ...
	     */
		    ...
		    //need to filter memento complient
	         ...
	         tgq.getTimegateInfo( archivelist, id, cdate , false); //
